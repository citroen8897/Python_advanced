...
def main():
    ...
    index_lines = [matrix.index(line) for line in matrix
                   if user_numero in line]

    matrix_transpose = [[line[j] for line in matrix] for j in
                        range(len(matrix))]

    index_rows = [matrix_transpose.index(line) for line in matrix_transpose
                  if user_numero in line]

    if len(index_lines) == 0:
        print(f'Число {user_numero} в матрице отсутствует\n')
    else:
        print(f'Число {user_numero} встречается в строках: '
              f'{", ".join(map(str, index_lines))}')
        print(f'Число {user_numero} встречается в столбцах: '
